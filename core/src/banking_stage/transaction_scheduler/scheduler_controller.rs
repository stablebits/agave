...
/// is allowed to drive saturation. This prevents publishing a stale floor
/// when the queue is near-empty (the min priority of a tiny set of
...
/// Saturation detection + pf-floor publication.
...
impl SaturationState {
    /// Caller must clear the floor at controller-construction time
    /// *unconditionally* so a stale floor from a prior controller can't
    /// keep sigverify dropping packets.
    ...
    S: Scheduler<R::Transaction>,
{
    pub fn new(
        ...
        // from a prior scheduler can't keep sigverify dropping packets.
    ...
    /// Update the scheduler-published priority floor based on incoming packet
    /// arrivals and the current queue state.
    ///
    /// Saturation is driven by a token bucket fed at a fixed TPS and consumed
    /// by actual arrivals. The bucket going empty under pressure is the
    /// primary "over-capacity" signal. It is AND-guarded with a minimum
    /// queue-occupancy check so that we only publish a floor when there is
    /// a meaningful set of in-queue transactions to derive it from
    /// (otherwise the min priority of a near-empty queue is arbitrary and
    /// the floor would be noise).
    ///
    /// **Published floor:** the queue-min tx's priority evaluated against
    /// `MAINNET_FEE_CONTEXT` (see
    /// `scheduler_priority::calculate_pf_drop_priority`). Sigverify uses the
    /// same function on arriving txs, so the floor and arrivals compare in
    /// identical units — independent of any drift between the live bank
    /// and mainnet defaults. The number we publish is *not* the
    /// bank-context priority the BTreeSet ranks by; for typical mainnet
    /// feature-set / fee inputs the two are within rounding, but using
    /// mainnet-context everywhere makes the comparison exactly consistent.
    ///
