...
impl Waker {
    ...
    /// Attempts to find another thread's entry and select the operation.
    ///
    /// The selected entry is returned; the *caller* is responsible for waking
    /// the thread (`entry.cx.unpark()`). Callers that hold a lock around this
    /// waker should release it first: the wake is a syscall, and performing
    /// it under the lock serializes all threads contending on the waker.
    /// The claim is exclusive either way -- `cx.try_select` is a CAS that can
    /// only succeed once, and a delayed unpark is absorbed by the park-token
    /// protocol (the woken thread re-checks its selected state).
    #[inline]
